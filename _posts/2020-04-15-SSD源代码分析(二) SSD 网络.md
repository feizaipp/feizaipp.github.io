...
# 2. SSD 网络
## 2.1. SSD 网络数据处理
&#160; &#160; &#160; &#160;图像数据在输入网络之前后之后都需要对图像进行处理才能达到往网络训练和预测的结构。
...
* w, h: 随机生成 w 和 h ，保证宽高比例在 0.5-2 之间
* left, top: left 范围为 [0, wtot - w]; top 范围为 [0, htot - h]
* right, bottom: right = left + w; bottom = top + h
* ious: 计算 bboxes 与 [left, top, right, bottom] 的 iou ，如果 iou 值不是所有的都大于 min_iou 或者都小于 max_iou 的化则重新生成随机数。
* masks: 查找所有的 gt box 的中心点有没有在随机采样的范围内，如果没有则重新采样
* bboxes[bboxes[:, 0] < left, 0]: 修改采样范围中的所有 gt box 的坐标，并防止越界情况
* bboxes = bboxes[masks, :]: 虑除不在采样范围中的 gt box
* left_idx, top_idx, right_idx, bottom_idx: 确定裁剪范围
* image.crop: 裁剪图像
* 调整裁剪后的 bbox
```
...
* best_dbox_idx[best_bbox_idx[idx]] = idx: 上面已经将每个 gt box 匹配到的最佳 Default Box 设置为正样本，这里就是设置对应的正样本的索引，也就是正样本的标签
...
```
&#160; &#160; &#160; &#160;SSD300 在上一篇文章中已经介绍了一部分，这篇文章从 Default Box 的生成开始。 Default Box 与 Faster-RCNN 网络中的 Anchor 类似，但是生成规则有些不同，线面我们介绍如何生成 Default Box 。
* figsize: 输入网络的图像的大小
* feat_size: 每个预测层的feature map尺寸
* step: 每个特征层上的一个 cell 在原图上的跨度
* scales: 每个特征层上预测的 Default Box 的 scale ，当前层的 Default Box 的尺度与 sacles[l] 和 scales[l+1] 的值共同决定
...
* mask: 保存正样本的 mask ， torch.gt(a, b) ，如果 a > b 则为 1 ，不大于 b 则为 0 ，预处理时，已将正样本对应的标签赋值
* pos_num: 计算每个 batch 中每张图片的正样本个数
* self._location_vec: 计算 GT box 相对 Default Box 的回归参数
* self.location_loss: 计算定位损失
* self.confidence_loss: 计算类别损失，只有正样本有定位损失，并且计算每个 batch 的损失和
