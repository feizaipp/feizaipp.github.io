...
## 2.3. v1 版本网络存在的问题
&#160; &#160; &#160; &#160;首先， v1 版本网络对一堆小目标检测效果不理想，比如一群鸟，因为 v1 版本中一个网格只检测两个目标。其次，模型很难泛化到新的、不常见的预测目标比例。最后，模型的损失函数会同样的对待小边界框与大边界框的误差，大边界框的小误差通常是正常的，但小边界框的小误差对 IOU 的印象要大得多。
...
def conv_batch(in_num, out_num, kernel_size=3, padding=1, stride=1):
    return nn.Sequential(
        ...
        nn.LeakyReLU())
```
&#160; &#160; &#160; &#160;残差层，降输入的特征经过 1x1 和 3x3 卷积后与输入特征相加， 1x1 卷积对特征层降维，然后通过 3x3 卷积再次将特征层升维到与输入一致，然后降两者进行相加。
```
